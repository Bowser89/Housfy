 ...
 * DeleteOffice.
 */
class DeleteOffice extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'offices:delete';
     ...
     * @var string
     */
    protected $description = 'Deletes an office.';
     ...
     */
    public function handle(OfficeRepository $officeRepository)
    {
        // User's input
        ...
        $this->info('About to delete office with the following values:');

        $tableHeader = ['Id', 'Name', 'Address'];
        $this->table($tableHeader, [$office->toArray()]);

        $userConfirmsOfficeDeletion = $this->confirm('Are you sure you want to proceed?');

        if ($userConfirmsOfficeDeletion) {
            try {
                $office->delete();
                $this->info('Office deleted successfully!');
                return 0;
            } catch (Exception $e) {
